...
// src/workers/local-explorer/explorer-refresh.ts
var EXPLORER_REFRESH_HEADER = "X-Miniflare-Explorer-Refresh";
function isAutomaticWorkersRefresh(method, routeName, headerValue) {
  return method === "GET" && routeName === "local.workers" && headerValue === "poll";
}
...
var zR2Messages = z2.array(z2.string()), zR2Errors = z2.array(
...
), zLocalExplorerScheduledRequest = z2.object({
  cron: z2.string().regex(/.*\S.*/),
  scheduled_time: z2.int().gte(-9223372036854).lte(9223372036854).optional()
}), zLocalExplorerScheduledResult = z2.object({
  outcome: z2.string(),
  noRetry: z2.boolean()
}), zR2Object = z2.object({
...
}), zLocalExplorerWorkerTriggers = z2.object({
  crons: z2.array(z2.string())
  ...
  isSelf: z2.boolean(),
  name: z2.string(),
  persistenceScope: z2.string().optional(),
...
), zLocalExplorerDispatchScheduledData = z2.object({
  body: zLocalExplorerScheduledRequest,
  path: z2.never().optional(),
  query: z2.object({
    worker: z2.string().min(1)
  })
}), zLocalExplorerDispatchScheduledResponse = zWorkersApiResponseCommon.and(
  z2.object({
...
var openapi_local_default = {
  ...
  paths: {
    ...
    "/local/scheduled": {
      post: {
        description: "Dispatches one scheduled invocation to the exact Worker available to Local Explorer under the requested name.",
    ...
    schemas: {
      ...
      "local-explorer_scheduled-request": {
        ...
        properties: {
          ...
          scheduled_time: {
            ...
            description: "Epoch milliseconds within workerd's signed 64-bit nanosecond range."
          }
        }
      },
      ...
      "local-explorer_worker": {
        ...
        properties: {
          ...
          persistenceScope: {
            type: "string",
            description: "Opaque stable identifier for the worker's local project, used to scope browser persistence without exposing its filesystem path"
          },
          bindings: {
...
function dispatchScheduled(service, options) {
  return service.scheduled(options);
}

// src/workers/local-explorer/resources/scheduled.ts
var SCHEDULED_ERROR_CODE = 1e4;
function isFetcher2(value) {
  return typeof value == "object" && value !== null && "scheduled" in value && typeof value.scheduled == "function";
}
function getUserWorkerService2(c, worker) {
  let service = c.env[`${CoreBindings.SERVICE_EXPLORER_USER_WORKER_PREFIX}${worker}`];
  return isFetcher2(service) ? service : void 0;
}
function peerUnavailable(message) {
  return errorResponse(502, SCHEDULED_ERROR_CODE, message);
}
async function dispatchLocalScheduled(c, worker, body) {
  let service = getUserWorkerService2(c, worker);
  if (service === void 0)
    return peerUnavailable(`The owner of Worker "${worker}" is unavailable.`);
  try {
    let result = await dispatchScheduled(service, {
      cron: body.cron,
      scheduledTime: body.scheduled_time === void 0 ? void 0 : new Date(body.scheduled_time)
    }), plainResult = JSON.parse(JSON.stringify(result));
    return Response.json(wrapResponse(plainResult));
  } catch (error) {
    let message = error instanceof Error ? error.message : `Failed to dispatch Worker "${worker}".`;
    return errorResponse(500, SCHEDULED_ERROR_CODE, message);
  }
}
async function decodePeerResponse(response, worker) {
  if (response === null)
    return peerUnavailable(`The owner of Worker "${worker}" is unavailable.`);
  let payload;
  try {
  ...
  return response.status === 200 ? zLocalExplorerDispatchScheduledResponse.safeParse(payload).success ? Response.json(payload) : peerUnavailable(
    `The owner of Worker "${worker}" returned an invalid response.`
  ) : (response.status === 500 || response.status === 502) && zWorkersApiResponseCommonFailure.safeParse(payload).success ? Response.json(payload, { status: response.status }) : peerUnavailable(`The owner of Worker "${worker}" is unavailable.`);
}
async function dispatchScheduledToWorker(c, query, body) {
  let forwarded = c.req.raw.headers.has(NO_AGGREGATE_HEADER), registryResponse = await c.env[CoreBindings.SERVICE_LOOPBACK].fetch(
    "http://localhost/core/dev-registry"
  ), registry = await registryResponse.json(), selfInstanceId = registryResponse.headers.get(
    "X-Miniflare-Dev-Registry-Instance-Id"
  ), owner = registry[query.worker];
  if (owner === void 0)
    return !forwarded && c.env[CoreBindings.JSON_LOCAL_EXPLORER_WORKER_NAMES].includes(
      query.worker
    ) ? dispatchLocalScheduled(c, query.worker, body) : forwarded ? peerUnavailable(`This instance does not own Worker "${query.worker}".`) : errorResponse(
      404,
  ...
  let params = new URLSearchParams({ worker: query.worker }), response = await fetchFromPeer(
...
async function telemetryMiddleware(c, next) {
  await next();
  let routeName = getRouteName(c.req.path);
  if (isAutomaticWorkersRefresh(
    c.req.method,
    routeName,
    c.req.header(EXPLORER_REFRESH_HEADER)
  ) || !c.res.ok || !c.env.MINIFLARE_TELEMETRY_CONFIG.enabled || // Skip telemetry for aggregation calls between instances
  c.req.raw.headers.has(NO_AGGREGATE_HEADER) || !c.env.MINIFLARE_TELEMETRY_CONFIG.deviceId)
    return;
  let route = `${routeName}.${c.req.method.toLowerCase()}`, properties = {
